...
let rec is_value = function
  | S.Cons (e1, e2) | S.Pair (e1, e2) -> (is_value e1) && (is_value e2)
...
let rec step = function
  ...
  | S.Fst (S.Pair (v1, v2)) when is_value (S.Pair (v1, v2)) -> v1 
  | S.Fst e -> S.Fst (step e)
  | S.Snd (S.Pair (v1, v2)) when is_value (S.Pair (v1, v2)) -> v2
  ...
  | S.Match (S.Cons (v, vs), e1, x, xs, e2) when ((is_value v) && (is_value vs)) -> S.subst [(x, v); (xs, vs)] e2 
